...
|v18.1.0 |내장함수 31종 추가                  |
|v18.5.0 |온톨로지 1~2회차 — kc_ontology.h/c + kc_ont_query.h/c + kc_ont_import.h/c|
|v19.0.0 |온톨로지 3회차 — kc_ont_import Turtle/OWL + kc_ont_server.h/c REST+WS|
|v20.0.0 |온톨로지 4회차 — kc_ont_remote.h/c + klexer 토큰 10종 + kparser AST 노드 6종|

-----

## ✅ v18.5.0 ~ v20.0.0 — 온톨로지 시스템 (1~4회차 완료)

> 지식 그래프 + SPARQL + MCP 자동 노출 — **4회차까지 완료**

|단계|내용                                      |상태|
|--|----------------------------------------|--|
|1 |kc_ontology.h/c — 그래프 CRUD + 추론기        |✅ 완료 (v18.5.0) |
|2 |kc_ont_query.c — 한글 질의 + SPARQL         |✅ 완료 (v18.5.0) |
|3 |kc_ont_import.c — JSON-LD / CSV / Turtle|✅ 완료 (v18.5.0~v19.0.0) |
|4 |kc_ont_server.c — REST + WebSocket      |✅ 완료 (v19.0.0) |
|5 |kc_ont_remote.h/c — 외부 서버 클라이언트         |✅ 완료 (v20.0.0) |
|6 |klexer/kparser 온톨로지 토큰/노드 연동       |✅ 완료 (v20.0.0) |
|7 |MCP 자동 노출                               |⬜ 대기 (v20.1.0 — 5회차) |

## 🔄 v20.1.0 — 온톨로지 5회차 (다음 작업)

> kinterp.c 3모드 블록 실행 + MCP 자동 노출

|단계|내용|상태|
|--|--|--|
|9 |`kinterp.c` — `온톨로지` 블록 3모드(내장/대여/접속) 실행 구현|⬜ **다음** |
|10|MCP 자동 노출 — 모드 1/3에서 온톨로지 항목 MCP 도구 자동 등록|⬜ 대기 |
|- |gcc -fsyntax-only 검증|⬜ 대기 |

> **버전 정책**: kinterp.c 1~2파일 수정 → **v20.1.0** (중간 자리 올림)
...
```
v18.1.0  내장함수 31종              ✅ 완료
v18.5.0  온톨로지 1~2회차          ✅ 완료 (kc_ontology + kc_ont_query + kc_ont_import)
v19.0.0  온톨로지 3회차            ✅ 완료 (Turtle/OWL + kc_ont_server REST+WS)
v20.0.0  온톨로지 4회차            ✅ 완료 (kc_ont_remote + klexer/kparser 토큰/노드)
v20.1.0  온톨로지 5회차            🔄 다음 (kinterp 3모드 실행 + MCP 자동 노출)
v21.0.0  온톨로지 6~8회차          ⬜ 예정 (SDK + kcodegen + 통합 테스트)
v20.5.0  Concept Identity         ⬜ 예정 (의미→벡터→실행→검증→기억)
         / Vector Space
v21.0.0  지식 뱅크                 ⬜ 예정 (학습 지식 저장·이식·외부AI공유)
v22.0.0  WEB IDE 기본              ⬜ 예정 (레이아웃+에디터+WASM실행)
v22.5.0  블록 캔버스               ⬜ 예정 (시각적 블록 편집기)
v23.0.0  표준 UI 객체              ⬜ 예정 (위젯 라이브러리 32종)
v23.5.0  이미지 처리               ⬜ 예정 (로드/조작/AI연동)
v24.0.0  렌더링 시스템             ⬜ 예정 (2D/3D/애니메이션/물리)
v24.5.0  PC 서버화                 ⬜ 예정 (REST/MCP/WS 자동 생성)
v25.0.0  패키지 생태계             ⬜ 예정 (모듈 레지스트리+FFI+SDK)
```
...
|중규모 추가 (3개 미만 파일)|v x.0.0 → v x.1.0  |
|대규모 (3파일 이상)  |v x.x.x → v (x+1).0.0|

> 메인 파일(readme.md) 기준으로 버전을 올리고, 각 파트 파일은 메인의 버전을 상속한다.

-----

*Kcode v20.0.0 — 한글 AI 제어 언어*  
*언어 엔진 → 온톨로지 K엔진(4/8회차 완료) → WEB IDE → UI/렌더링 → 생태계*